...
void Graphics::loadTraitTextures(TraitPackage* tPack)
{
    ...
    tPack->traitTextures.insert({ TraitTag::enumRage , texture });
    if (!texture.loadFromFile("textures\\traits\\AttachedTrait.png"))
    {
        throw new std::exception("could not load menu image");
    }
    tPack->traitTextures.insert({ TraitTag::enumAttached , texture });
    if (!texture.loadFromFile("textures\\traits\\FormationTrait.png"))
    {
        throw new std::exception("could not load menu image");
    }
    tPack->traitTextures.insert({ TraitTag::enumFormation , texture });
    if (!texture.loadFromFile("textures\\traits\\InspiringHeroismTrait.png"))
    {
        throw new std::exception("could not load menu image");
    }
    tPack->traitTextures.insert({ TraitTag::enumInspiringHeroism , texture });
