...
ActionAnimation_Strike::ActionAnimation_Strike(BattleAnimation* battleAnim, ActionResults results, bool isMaster, GraphicsAssets* assets, sf::RenderWindow* ww) : ActionAnimation(battleAnim, results, isMaster, assets, ww)
{
	unsigned int i;
	nrOfStrikesToRun = instanceLocks.size();
	/*int biggestNr = -1;
	for (i = 0; i < results.getEffects().size(); i++)
	{
		int iNr = results.getEffects().at(i).instanceNumber;
		if (iNr > biggestNr)
		{
			biggestNr = iNr;
		}
	}
	nrOfStrikesToRun = biggestNr;*/
	nrOfStrikesRan = 0;
}

bool ActionAnimation_Strike::progressAction()
{
	unsigned int i;
